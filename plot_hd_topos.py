...
# No removal as sigma band only(10-15Hz)
...
stag1, stag2 = (mspe_stag1, mspe_stag2)
...
def plot_topo_psd(psd_to_plot, stag, what_stag, fmin, fmax, info, week=None):
    ...
    tfr.plot_topomap(axes=ax, cbar_fmt='%3.1f',  unit='', contours=0)
    ax.set_title(week)
    plt.tight_layout()
    plt.show()
what_stag=1

plot_topo_psd(psd1, stag1, what_stag=what_stag, fmin=22, fmax=23,  info=info, week= 'week2')
plot_topo_psd(psd2, stag2, what_stag=what_stag, fmin=1, fmax=2, info=info, week= 'week5')
...
def get_av_psd(psd_to_plot, stag, what_stag, info, fmin, fmax, week=None):
    ...
    #freqs_mask = np.logical_and(freqs>=fmin, freqs<=fmax) #mask to select bins
    #psd_data = psd_data[freqs_mask, :, :].sum(0)[np.newaxis, :,:] #sum power for selected bins
    psd_data = psd_data.sum(0)[np.newaxis, :,:] #sum power for selected bins
    psd_data = psd_data.transpose([1,0, 2])
    #psd_data = psd_data*100. #convert to %
    return psd_data
