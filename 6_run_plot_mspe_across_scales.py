...
scale_idx = [0,1,2,3,4] #all scales
drop_outliers = False
...
def aggregate_for_given_scale(scale_idx):
    store = {'week2':[], 'week5':[]}
...
for scale in scale_idx:
    d = aggregate_for_given_scale(scale)
    tmp.append(d)

data = pd.concat(tmp)

fig, axes = plt.subplots(1,3, sharey=True)
colors = {'week2':'black', 'week5':'red'}
for i, s in enumerate(['NREM', 'REM', 'WAKE']):
    d = data[data['stag']==s]
    legend = True if s == 'WAKE' else False
    print(legend)
    sns.catplot(x='scale', y='value', hue='time', data=d, ax=axes[i],  capsize=.2,
                height=6, aspect=.75, kind='point', sharey=True, palette=colors)

    axes[i].get_legend().remove() if i != 2 else None
    ylab = 'MSPE [bit]' if i == 0 else ''
    axes[i].set_ylabel(ylab)
    axes[i].set_xlabel('Scale')
    plt.savefig('mspe_scales.tif', dpi=300)
